---
name: gzh-layout-cloner
description: 克隆任意公众号文章的排版风格，套到用户自己的内容，输出可直接粘贴公众号的 inline-style HTML。两段式——【提取】给一篇喜欢的公众号文章 URL→自动归纳组件架构→渲染预览→确认后存成命名模板；【套用】用某个已存模板把用户的 markdown 渲染成 HTML。触发：「给个链接克隆排版」「提取这篇的排版」「用 XX 模板生成」「gzh-layout-cloner」。
---

# 公众号排版克隆器（gzh-layout-cloner）
...
项目的内置公众号渲染器在 `/Users/doushun/Desktop/workspace/proj04-obsidian-publisher/web/index.html`，线上入口为 `https://alinefan.github.io/doc-to-social-render/`。遇到「把笔记排成公众号」这类常规需求，优先走内置模板；只有用户提供参考公众号链接并明确要复刻风格时，才进入下文的提取流程。

- **默认模板：绿色科技**。除非用户明确指定，公众号一律先用它。
- 可选模板：`苹方周刊`、`极简墨白`、`极简代码`、`莫兰迪`。它们是内部模板，不取代绿色科技的默认地位。
- 文章中的 `# 一级标题` 是正文排版的一部分，必须保留并按模板渲染；**不要**因文件名、frontmatter 或平台文章标题而额外注入一个 H1，也不要把原有 H1 删除。
- 点击“复制到公众号”前，确认图片已作为可复制内容处理；最终粘贴到公众号编辑器后检查图片是否进入素材库。

内置模板的选择规则：用户未指定时选绿色科技；用户指定模板名时按原名选择；用户要“参考某篇文章”或需要新视觉语言时，走 §A 提取并把结果作为新增模板保存，绝不覆盖已有模板。

## 🚧 边界：克隆的是 CSS 排版，不是图片设计

| ✅ 能克隆（CSS 写出来的） | ❌ 不能克隆（图片化的设计） |
|---|---|
| 文字样式、CSS 文字标题、各级标题、强调、代码块、列表、引用、卡片/封面卡、图框边框… | **标题 / 编号 / 整版做成 PNG 的**——图里文字焊死像素、没有 CSS 可抠 |

- 图片化标题这类：提取**抓住图地址 + 标 `is_image`**，但**没有"标题样式"可提取**（是图不是字）；**多图文章里也无法可靠判断哪张图是标题**。
- 用不用这些图由**用户在套用时决定**（复刻同篇可嵌原图 / 套新内容需换你自己的图）。
- 一句话：标题是 **CSS 文字** → 完整克隆、可套你的字；标题是**图** → 只能抓住那张图、套用时换图。
...
| 用户说什么 | 走哪条 |
|---|---|
| 给一篇 md、未指定参考文章 | **内置模板路线**（默认绿色科技） |
| 指定绿色科技 / 苹方周刊 / 极简墨白 / 极简代码 / 莫兰迪 | **内置模板路线**（按指定模板） |
...
```bash
# 提取（阶段 A）
python3 skills/gzh-layout-cloner/extract_styles.py prep <url> <名>     # → inventory + cleaned.html
#   （subagent 读 prep 产物 + 本文提取契约 → 写 templates/<名>.json）
python3 skills/gzh-layout-cloner/extract_styles.py validate <名>        # 机器校验 schema/必备件/box-sizing
python3 skills/gzh-layout-cloner/extract_styles.py render <名>          # → templates/<名>-preview.html
#   （open 预览给用户确认）
