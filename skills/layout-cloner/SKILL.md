...
5. **★ 图类标 `is_image`，图片化设计如实登记**。
   - 编号图、配图、**图片化的标题 / 分节头 / 整版设计图**（很多公众号把大标题直接做成 PNG）——这些是**图片、不是 CSS 排版**，对应槽位标 `"is_image": true`。
   - 图片化的标题 / hero 登记成**单独组件**（如 `hero_image`），别和正文配图混成一类。
   - `structure_rules` 老实注明「**这是图、克隆不了**，套用户内容时由用户换图 / 重做，不可复用原图」。
   - ⚠ **边界**：本 skill 克隆的是 **CSS 排版**。若一篇文章的视觉设计主要在**图片**里（标题图、设计配图占多数），能克隆的 CSS 就很薄——**如实提取那薄薄一层，并明确告诉用户哪些是图、克隆不了**，绝不假装能克隆图片设计。预览会自动给图片组件打 ⚠ 标记 + 顶部摘要。
...
- **通用兜底**：模板里 `sample`/`example` = 参考示例，**仅认组件、绝不进产出**。`is_image:true` 的图**不能直接搬原文的图**，按用户内容重新出图或留占位。
